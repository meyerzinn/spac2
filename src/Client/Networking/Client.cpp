...
Client::Client(asio::io_context &ioc, ssl::context &ctx)
    : resolver_(asio::make_strand(ioc)),
      spac::net::Connection(websocket::stream<beast::ssl_stream<beast::tcp_stream>>(asio::make_strand(ioc), ctx)) {}
...
void Client::on_resolve(beast::error_code ec, tcp::resolver::results_type results) {
  ...
  BOOST_LOG_TRIVIAL(debug) << "resolved host" << std::endl << "opening connection" << std::endl;
  // set TCP timeout
...
void Client::on_connect(beast::error_code ec, const tcp::resolver::results_type::endpoint_type &) {
  ...
  ws_.set_option(websocket::stream_base::decorator(
      [](websocket::request_type &req) { req.set(http::field::user_agent, std::string(BOOST_BEAST_VERSION_STRING)); }));
...
void Client::on_handshake(beast::error_code ec) {
  ...
  BOOST_LOG_TRIVIAL(debug) << "handshake complete" << std::endl << "websocket connection established" << std::endl;
